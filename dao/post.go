...
func GetPostById(pid int) (models.Post, error) {
	...
	//fmt.Println("######################################")
	//fmt.Printf("数据库映射到这里,文章title:%s\n", post.Title)
	//fmt.Printf("数据库映射到这里,文章id:%d，文章主人id：%d\n", post.Pid, post.UserId)
	//fmt.Printf("数据库映射到这里,view:%d，CreateAt：%v\n", post.ViewCount, post.CreateAt)
	//fmt.Println("######################################")
...
func UpdateViewCount(pid, count int) {
	_, err := DB.Exec("update blog_post set  view_count= ? where pid=?", count, pid)
	if err != nil {
		log.Printf("数据库中更新查看次数失败err:%v\n", err)
		return
	}
}
